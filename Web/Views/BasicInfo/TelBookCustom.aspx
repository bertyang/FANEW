    ...
    <%: Scripts.Render("~/bundles/js")%>
        <script type="text/javascript" language="javascript">
            $(function () {
                ...
                $('#btnAdd').bind('click',
                    function () {
                        window.location.href = "/BasicInfo/TelBookCustomEdit/";
                    }
                );
                ...
                $('#btnEdit').bind('click',
                    function () {
                        ...
                        if (row) {
                            window.location.href = "/BasicInfo/TelBookCustomEdit/?id=" + row.ID;
                        }
                        else {
                ...
                $('#btnSort').bind('click',
                    function () {
                        ...
                        $('#sort').dialog({
                            ...
                            onClose:function(){
                                $('#txt_Type').combotree({
                                    url: '/BasicInfo/TypeTreeCustomLoad/'
                                });
                            },
                            buttons: [
                            {
...
<body class="easyui-layout"> 
    <div region="center" style="padding: 5px" border="true" > 
        <table id="grid" class="easyui-datagrid" align="center" toolbar="#tb" url="/BasicInfo/TelBookCustomLoad/"
                     ...
                     <div region="right">
                         <table width="100%">
                             <tr>
                                 ...
                                 <td width="100px">
                                     电话分类:
                                     <input class="easyui-combotree" style="width: 140px;" url="/BasicInfo/TypeTreeCustomLoad/"
                                         valuefield="编码" textfield="名称" id="txt_Type" value="--请选择--" editable="false" />
                                 </td>
                                 <td width="100px">
                                     <a href="#" class="easyui-linkbutton" iconcls="icon-search" onclick="javascript:$('#grid').datagrid('options').url = '/BasicInfo/TelBookCustomLoad/?Name=' + escape($('#txt_Name').val())
                                        + '&&Type=' + escape($('#txt_Type').combotree('getValue')) + '&&OwnerID=' + '<%=ViewData["OwnerID"] %>';
                                        var p1 = $('#grid').datagrid('getPager');$(p1).pagination({ pageNumber : 1,});
                                        $('#grid').datagrid('options').page = 1;$('#grid').datagrid('reload');">查询</a>
                                 </td>
                             </tr>
                         </table>
                     </div>      
    ...
    <div id="sort" region="center" border="true" style="overflow-y: hidden" title="分类管理">
        <iframe id="frame" scrolling="auto" frameborder="0" src="/BasicInfo/TelTypeCustom/" style="width: 100%;
            height: 100%;"></iframe>
    </div>
</body>
</html>
