...
{
    public class ActivityInstance : Instance 
    {
        #region 属性
        ...
        public void Create(int flowInstId, int activityId)
        {
            ...
            m_ActivityInstanceId = prikey.getIntPrimaryKey("F_INST_ACTIVITY");
        ...
        public void AssignWork()
        {
            ...
            if (listWorkerId.Count > 0)
            {
                ...
                if(this.Activity.IsSms == "Y" )
                {
                    ...
                    {
                        IInnerCommBLL innerCommBLL = ctx["InnerCommBll"] as IInnerCommBLL;

                        string content = string.Format("您有一笔{0}需要审核", FlowInstance.FlowDefine.Name);

                        Log4Net.LogInfo(content);

                        innerCommBLL.SendSMG(listMobile, content, "99999");
                    }
                }
            }
            else
